...
st.title("🖥️ Simulador de S.O. (CPU y Memoria)")
st.markdown("---")
...
modulo = st.sidebar.radio("Seleccione el Recurso a Simular:", ["Planificación de CPU", "Gestión de Memoria"])
archivo_subido = st.sidebar.file_uploader("Cargar lote de procesos (CSV)", type=["csv", "txt"])
...
# --- FUNCIONES GRÁFICAS ADAPTADAS PARA SCROLL HORIZONTAL (150+ PROCESOS) ---

def dibujar_gantt_escalable(gantt_data, max_t, titulo="Diagrama de Gantt", modo_versus=False, t_limite=None):
    # Si estamos simulando el paso del tiempo, adaptamos los datos reflejados en la gráfica
    if t_limite is not None:
        datos_filtrados = []
        for b in gantt_data:
            if b['Inicio'] < t_limite:
                # Si el bloque excede el instante de tiempo actual de la simulación, lo recortamos
    ...
    # Calculamos un ancho real en píxeles para el contenedor web
    ancho_px = max(1200, int(max_t * 6)) 
    if ancho_px > 15000: ancho_px = 15000 
    
    # Dimensiones lógicas en pulgadas para Matplotlib
    ...
    # Conversión a buffer de memoria como PNG base64
    buf = io.BytesIO()
    plt.savefig(buf, format='png', bbox_inches='tight')
    plt.close(fig)
    data = base64.b64encode(buf.getvalue()).decode('utf-8')
    
    # Inyección HTML con scroll horizontal
    html_gantt = f"""
...
if archivo_subido:
    ...
    if df_input is not None:
        ...
        if modulo == "Planificación de CPU":
            if modo_vs:
                ...
                # --- ⏱️ INSPECCIÓN DINÁMICA DE LA LÍNEA DE TIEMPO (NUEVO) ---
                st.markdown("---")
    ...
    # --- 📘 GUÍA DE USO INTERACTIVA CON FORMATO DE ARCHIVOS ---
    ...
    <div class="guide-box">
        ...
        <h5>Como usar el simulador</h5> 
        <P>👈 En panel izquierdo, seleccione la opcion a simular (CPU o Memoria), luego cargue un archivo CSV/TXT, selecciones el algoritmo a evaluar (o active el modo Versus para comparar todos) y explore los resultados detallados.</p>
        <P>Tambien puede usar la simulacion paso a paso para inspeccionar la evolución temporal de la CPU o el estado de la RAM a medida que se procesan los eventos.</P>
        <p>Para asegurar un correcto mapeo y procesamiento en los hilos del simulador y no este alucinando, verifique que su archivo plano contenga exactamente las cabeceras requeridas según el módulo a evaluar:</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_g1:
        ...
        st.write("El archivo debe incluir el identificador del proceso, su tiempo de arribo a la cola de listos y la ráfaga de ejecución en CPU.")
        ...
        st.markdown("**Ejemplo en texto plano (CSV / TXT):**")
        st.code("Proceso,T. Llegada,Duración\nP1,0,3\nP2,1,5\nP3,3,2\nP4,5,4", language="text")
        
    with col_g2:
        st.markdown("#### 🧠 Formato para Gestión de Memoria")
        st.write("Además de las ráfagas temporales, el archivo debe incluir la demanda de almacenamiento físico (en Kilobytes 'K') para los bloques de partición.")
